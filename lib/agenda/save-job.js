...
const processDbResult = (job, result) => {
  ...
  if (res) {
    ...
    // Clear tracking of attrs value changes, see warpAttrs() in new Job()
    job.attrs._clearChanges();
...
module.exports = async function(job) {
  try {
    ...
    const now = new Date();
    const protect = {};
    let propsUpdate = props;
    if (id) {
      // If the job already had an ID, then we only need to update those props that have been changed
      propsUpdate = {};
      for (const p of job.attrs._changedProps) {
        propsUpdate[p] = props[p];
      }
    }
    let update = {$set: propsUpdate};
