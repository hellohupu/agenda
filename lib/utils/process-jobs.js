...
module.exports = function(extraJob) {
  ...
  if (!extraJob) {
    // Go through each jobName set in 'Agenda.process', clear expired locks and fill the queue with the next jobs
    for (jobName in definitions) {
      if ({}.hasOwnProperty.call(definitions, jobName)) {
        clearExpiredLocks(jobName, (err, jobName) => {
          // Fill the queue with the next jobs, no matter whether errors have occurred when clearing expired locks
          debug('queuing up job to process: [%s]', jobName);
          jobQueueFilling(jobName);
          // Throw the errors so that we can be aware
          if (err) {
            debug('error occurred when clearing expired locks of jobs: [%s]', jobName);
            throw err;
          }
        });
      }
  ...
  function clearExpiredLocks(jobName, cb) {
    debug('clearing expired locks of jobs: [%s]', jobName);
    const lockDeadline = new Date(Date.now() - definitions[jobName].lockLifetime);
